...
exports.parseMedicationList = parseMedicationList;
exports.lookupDrug = lookupDrug;
exports.checkDrugInteractions = checkDrugInteractions;
exports.formatDrugInfo = formatDrugInfo;
exports.formatInteractionWarnings = formatInteractionWarnings;
exports.analyzeMedicationSafety = analyzeMedicationSafety;
exports.getMedicalContext = getMedicalContext;
...
function parseMedicationList(medications) {
    return medications
        .split(/,|;|\n/)
...
async function checkDrugInteractions(medications) {
    ...
    if (names.length < 2)
        return [];
    const warnings = [];
    await Promise.all(names.map(async (drugName) => {
        const enc = encodeURIComponent(drugName.toLowerCase());
        const data = await safeFetch(`${FDA_BASE}/drug/label.json?search=openfda.generic_name:"${enc}"&limit=1`);
...
function extractInteractionSnippet(text, drugName) {
...
function inferSeverity(text) {
    const lower = text.toLowerCase();
    if (/fatal|life.threat|hemorrhag|bleed|severe|contraindic/i.test(lower))
        return "high";
    if (/increas|decreas|monitor|adjust|caution|avoid/i.test(lower))
        return "moderate";
    return "low";
}
// Format OpenFDA result for WhatsApp message
function formatDrugInfo(drug, lang = "en") {
    const lines = [];
    if (drug.recall) {
        lines.push(lang === "ms"
    ...
    if (drug.drugClass) {
        lines.push(lang === "ms" ? `💊 Kelas ubat: ${drug.drugClass}` : `💊 Drug class: ${drug.drugClass}`);
    }
    if (drug.warning) {
        lines.push(lang === "ms" ? `⚠️ Amaran FDA: ${drug.warning}` : `⚠️ FDA warning: ${drug.warning}`);
    }
    if (lines.length === 0)
        return "";
...
function formatInteractionWarnings(warnings, lang = "en") {
...
async function analyzeMedicationSafety(identifiedDrug, recentSymptoms, condition, allMedications, patientName, lang = "en") {
...
- Keep response under 150 words`;
    try {
        ...
        if (!text || text.includes("No specific concerns flagged"))
            return "";
        const header = lang === "ms"
...
async function getMedicalContext(identifiedDrug, allMedications, lang = "en") {
