...
exports.shouldAskWellness = shouldAskWellness;
exports.buildWellnessCheck = buildWellnessCheck;
exports.buildWellnessResponse = buildWellnessResponse;
function shouldAskWellness(lastWellnessCheck) {
    if (!lastWellnessCheck)
        return true;
    const daysDiff = (Date.now() - new Date(lastWellnessCheck).getTime()) / (1000 * 60 * 60 * 24);
    return daysDiff >= 3;
}
function buildWellnessCheck(caregiverName, lang = "en") {
    if (lang === "ms") {
        return `\n\n💙 ${caregiverName}, macam mana keadaan ANDA hari ini?\n\n1. Saya okay\n2. Sedikit penat\n3. Sangat tertekan`;
    }
    return `\n\n💙 ${caregiverName}, how are YOU holding up today?\n\n1. I'm okay\n2. A bit tired\n3. Really struggling`;
}
function buildWellnessResponse(choice, caregiverName, lang = "en") {
    if (lang === "ms") {
        const responses = {
