...
exports.downloadTwilioImage = downloadTwilioImage;
exports.analyzeImage = analyzeImage;
const genkit_1 = require("./genkit");
const medical_api_1 = require("./medical-api");
const cloud_vision_1 = require("./cloud-vision");
...
async function analyzeImage(ctx, imageBase64, mimeType) {
    ...
    let visionContext = "";
    try {
        const cvResult = await (0, cloud_vision_1.analyzeWithCloudVision)(imageBase64);
        visionContext = (0, cloud_vision_1.buildVisionContext)(cvResult);
    }
    catch (e) {
...
⚠️ Disclaimer: KAI Eyes provides visual guidance only — not a clinical diagnosis. Always consult a healthcare professional for definitive assessment.`;
    ...
    if (identifiedMatch) {
        ...
        const [medContext, safetyInsight] = await Promise.all([
            (0, medical_api_1.getMedicalContext)(identifiedDrug, ctx.medications ?? "", lang),
            ctx.recentSymptoms && ctx.recentSymptoms.length > 0
                ? (0, medical_api_1.analyzeMedicationSafety)(identifiedDrug, ctx.recentSymptoms, ctx.condition ?? "", ctx.medications ?? "", ctx.patientName, lang)
                : Promise.resolve(""),
        ]);
        if (medContext)
            result += medContext;
